...
# outNoPage.close()
    ...
    tabooList = list()#这个列表装超过200 的类和重合率大的类
    ...
    print('Dump Start')
    f1 = open('../data/taboolist.pkl', 'wb')
    pickle.dump(tabooSet, f1, True)
    f1.close()
    print('Dump Done')

def loadAll():
    #returns vocab, embd, y, x, x_other
    print("load taboo start")
    f = open('../data/taboolist.pkl', 'rb')
    tabooSet = pickle.load(f)
    f.close()
    print("load taboo done!")
    return tabooSet
